...
/** Colores por rol, convención fantacalcio: P ámbar, D verde, C celeste, A rojo. */
  ...
  return (
    <span className="inline-flex items-center gap-2">
      <span
        ...
        aria-label={ROLE_NAMES[role]}
      >
        {role}
      </span>
      {full && <span className={`${ROLE_STYLES[role].text} text-sm`}>{ROLE_NAMES[role]}</span>}
    </span>
  );
}
